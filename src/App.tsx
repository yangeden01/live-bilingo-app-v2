...
import { Radio, Youtube, Code2, Smartphone, Cpu, CheckCircle2, Sparkles, Volume2, ShieldCheck, Download, ListMusic, BookOpen, RefreshCw, Copy, Play, Pause, Sun, Moon, Bell, Power, ArrowUp } from 'lucide-react';
...
export default function App() {
  ...
  // Floating Watermark Button visibility: shows when scrolled past reading mode (4 tabs scrolled up)
  const [showBackToReadingFab, setShowBackToReadingFab] = useState(false);

  useEffect(() => {
    if (activeTab !== 'app' || contentSource !== 'radio') {
      setShowBackToReadingFab(false);
      return;
    }

    const checkScrollState = () => {
      const tabsEl = document.getElementById('subtitle-tabs-bar');
      const header = document.querySelector('header');
      const headerBottom = header ? header.getBoundingClientRect().bottom : 56;
      ...
      // When the top of tabsEl is scrolled above the sticky header bottom by more than 20px,
      // the user has pulled down the screen to read older content, thus leaving the 4-tab reading mode view.
    ...
    <div className={`min-h-screen font-sans selection:bg-blue-600 selection:text-white pb-12 transition-colors duration-200 ${appBgClass}`}>
      ...
      >
        <div className="max-w-6xl mx-auto px-3 sm:px-6 py-2 flex flex-nowrap items-center justify-between gap-2">
          ...
          {/* Right: Sleep Timer + Play/Pause Button on the Far Right (Ergonomic Single-hand control) */}
          <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
            {/* Top Sleep Timer Trigger Button */}
            <button
              type="button"
      ...
      {/* Floating Watermark Back-to-Reading-Mode Button (Fixed at bottom right when scrolled down) */}
      <AnimatePresence>
        {showBackToReadingFab && activeTab === 'app' && contentSource === 'radio' && (
          <motion.button
            key="back-to-reading-fab"
            ...
            title="回到閱讀模式（置頂「即時 / 紀錄 / 收藏 / 生詞」分頁）"
            aria-label="回到閱讀模式（4個分頁置頂）"
            id="back-to-reading-mode-fab"
            className={`fixed right-5 sm:right-7 z-40 p-2.5 sm:p-3 rounded-2xl border backdrop-blur-md cursor-pointer flex flex-col items-center justify-center gap-0.5 select-none transition-colors duration-150 group shadow-2xl ${
              effectiveTheme === 'paper'
                ? 'bg-[#FFFDF7]/85 border-[#C8B282]/90 text-[#3B2E1E] hover:bg-[#FFFDF7] shadow-[0_10px_28px_rgba(59,46,30,0.22)]'
                : effectiveTheme === 'light'
                ? 'bg-white/85 border-slate-300/90 text-slate-800 hover:bg-white shadow-[0_10px_28px_rgba(0,0,0,0.16)]'
                : 'bg-slate-900/85 border-slate-700/90 text-slate-100 hover:bg-slate-800 shadow-[0_10px_28px_rgba(0,0,0,0.65)]'
            }`}
            style={{
          ...
          >
            ...
            <span className="text-[9px] sm:text-[10px] font-black tracking-tighter leading-none opacity-85 group-hover:opacity-100 whitespace-nowrap">
              置頂分頁
            </span>
          </motion.button>
        )}
