...
import { discoverLiveEnglishNews } from './server/youtubeNewsDiscoveryService';
...
function resolveTargetStreamUrl(inputUrl: string): string {
  ...
  // Automatic normalization for ABC News AAC stream to 100% Groq-compatible MP3 stream
  if (target.includes('abc.streamguys1.com') || target.includes('newsradio/icecast.audio')) {
    target = 'https://playerservices.streamtheworld.com/api/livestream-redirect/WABCAM.mp3';
  }
  // Automatic normalization for Sky News AAC stream to 100% Groq-compatible MP3 stream
  if (target.includes('NOVA_SKYNEWSAAC.aac')) {
    target = 'https://playerservices.streamtheworld.com/api/livestream-redirect/NOVA_SKYNEWS.mp3';
  }
  // Automatic normalization for LiveNOW FOX AAC stream to 100% Groq-compatible MP3 stream
  if (target.includes('foxnewsradioaac')) {
    target = 'https://playerservices.streamtheworld.com/api/livestream-redirect/KTELAM.mp3';
  }
  // Automatic normalization for NBC News AAC stream to 100% Groq-compatible MP3 stream
  if (target.includes('stream.revma.ihrhls.com/zc6043')) {
    target = 'https://tunein.cdnstream1.com/2868_96.mp3';
  }
...
// Live English News Discovery Endpoint (Strict 12h window, caption validation, no Gemini in discovery)
app.get('/api/youtube/news', async (req, res) => {
  const forceRefresh = req.query.refresh === 'true' || req.query.refresh === '1';
  try {
    const result = await discoverLiveEnglishNews(forceRefresh);
    res.json(result);
  } catch (error: any) {
    console.error('[YouTubeNews] Discovery route error:', error);
    res.status(500).json({
      success: false,
      code: 'YOUTUBE_API_UNAVAILABLE',
      message: error?.message || 'YouTube 美語新聞探索暫時無法連線，請稍後重試',
      videos: [],
    });
  }
});
...
app.post('/api/notify-station-playing', (req, res) => {
  ...
  if (url && typeof url === 'string') {
    ...
    currentRadioStationName = stationDisplayName;
...
export const YOUTUBE_LIVE_STREAMS = [
  {
    ...
    category: '英國國際',
    badge: 'BBC News',
    keywords: ['bbc', 'sc21UUjyLaw', 'world service', 'bbc news', 'british'],
  ...
  },
  {
    ...
    category: '國際時事',
    badge: 'ABC News',
    keywords: ['abc', 'jUkT0JL0Cjg', 'vOTiJkg1voo', 'australia', 'abc news', 'wabc'],
  },
  {
    id: 'bloomberg-live',
...
function getActiveGroqModel(preferred?: string, stationUrl?: string): string {
  ...
  const turboExhausted =
    ...
    (turboUsage && (turboUsage.requestsToday >= 2000));
  const turboInCooldown = now < groqTurboCooldownUntil;

  const v3Usage = sttUsageTracker?.groqModelsUsage?.[MODEL_V3];
  const v3Exhausted =
    groqV3DailyQuotaExhausted ||
    (v3Usage && (v3Usage.requestsToday >= 2000));
  const v3InCooldown = now < groqV3CooldownUntil;

  // Prefer model that is neither exhausted nor in cooldown
  if ((turboExhausted || turboInCooldown) && !v3Exhausted && !v3InCooldown) {
    return MODEL_V3;
  }
  if ((v3Exhausted || v3InCooldown) && !turboExhausted && !turboInCooldown) {
    return MODEL_TURBO;
  }
  ...
  // Both models are healthy: prioritize Turbo as primary free engine
  ...
  if (targetUrl) {
    ...
    if (cachedAssignment === MODEL_TURBO && turboRpm < 18 && !turboInCooldown) return MODEL_TURBO;
    if (cachedAssignment === MODEL_V3 && v3Rpm < 18 && !v3InCooldown) return MODEL_V3;
    ...
    if (turboRpm >= 18 && v3Rpm < 18 && !v3InCooldown) {
      selectedModel = MODEL_V3;
    } else if (v3Rpm >= 18 && turboRpm < 18 && !turboInCooldown) {
      selectedModel = MODEL_TURBO;
    } else if (turboStations > v3Stations && !v3InCooldown) {
      selectedModel = MODEL_V3;
    } else {
      selectedModel = MODEL_TURBO;
    }
  ...
  // Default distribution: Turbo primary, fallback to V3 only when Turbo hits high RPM
  return (turboRpm < 18 && !turboInCooldown) ? MODEL_TURBO : MODEL_V3;
}
...
interface SttApiUsageTracker {
  ...
  deepgramResetAt: string; // ISO string recording last manual reset time
  deepgramRequestsSinceReset: number;
  deepgramStreamSecondsSinceReset: number;
}
...
const sttUsageTracker: SttApiUsageTracker = {
  ...
  deepgramResetAt: new Date().toISOString(),
  deepgramRequestsSinceReset: 0,
  deepgramStreamSecondsSinceReset: 0,
};
...
function loadSttUsageTracker() {
  try {
    if (fs.existsSync(STT_USAGE_CACHE_FILE)) {
          ...
          for (const [m, info] of Object.entries(data.groqModelsUsage as Record<string, any>)) {
            if (sttUsageTracker.groqModelsUsage[m]) {
              ...
              if (info.remainingRequests) {
                ...
                const isDailyQuotaReset = (info.resetRequests && /h|d/i.test(info.resetRequests)) || ((info.requestsToday || 0) >= 2000);
                if (!isNaN(remNum) && remNum > 0 && !isDailyQuotaReset) {
                  if (m === 'whisper-large-v3-turbo') groqTurboDailyQuotaExhausted = false;
                  if (m === 'whisper-large-v3') groqV3DailyQuotaExhausted = false;
                } else if (remNum === 0 && isDailyQuotaReset) {
      ...
      // Deepgram is manually reset by user (persists across days to track billing periods)
      if (data) {
        if (data.deepgramResetAt) sttUsageTracker.deepgramResetAt = data.deepgramResetAt;
        if (typeof data.deepgramStreamSecondsSinceReset === 'number') {
          sttUsageTracker.deepgramStreamSecondsSinceReset = data.deepgramStreamSecondsSinceReset;
        } else if (typeof data.deepgramStreamSecondsToday === 'number') {
          sttUsageTracker.deepgramStreamSecondsSinceReset = data.deepgramStreamSecondsToday;
        }
        if (typeof data.deepgramRequestsSinceReset === 'number') {
          sttUsageTracker.deepgramRequestsSinceReset = data.deepgramRequestsSinceReset;
        } else if (typeof data.deepgramTodayRequests === 'number') {
          sttUsageTracker.deepgramRequestsSinceReset = data.deepgramTodayRequests;
        }
        if (typeof data.deepgramTotalRequestsEver === 'number') {
          sttUsageTracker.deepgramTotalRequestsEver = data.deepgramTotalRequestsEver;
        }
        sttUsageTracker.deepgramStreamSecondsToday = sttUsageTracker.deepgramStreamSecondsSinceReset;
        sttUsageTracker.deepgramTodayRequests = sttUsageTracker.deepgramRequestsSinceReset;
      }
    }
  } catch (err) {
...
function recordGroqRequest(headers?: Headers, extra?: { latencyMs?: number; status?: number; model?: string }) {
  ...
  if (headers) {
    ...
    const isResetDaily = (resetReq ? /h|d/i.test(resetReq) : false) || (sttUsageTracker.groqModelsUsage[currentModel].requestsToday >= 2000);

    if (remReq !== null) {
      sttUsageTracker.groqModelsUsage[currentModel].remainingRequests = remReq;
      const parsedRem = parseInt(remReq, 10);
      if (!isNaN(parsedRem)) {
        if (parsedRem > 0 && !isResetDaily) {
        ...
        } else if (parsedRem === 0 && isResetDaily) {
    ...
    if (limitReq && remReq) {
      ...
      if (!isNaN(parsedLimit) && !isNaN(parsedRem)) {
        ...
        } else if (parsedLimit >= parsedRem && parsedRem > 0 && !isResetDaily) {
          if (currentModel === 'whisper-large-v3-turbo') groqTurboDailyQuotaExhausted = false;
          if (currentModel === 'whisper-large-v3') groqV3DailyQuotaExhausted = false;
...
function recordDeepgramRequest(durationSec: number = 3.5) {
  const now = Date.now();
  sttUsageTracker.deepgramRequestsSinceReset = (sttUsageTracker.deepgramRequestsSinceReset || 0) + 1;
  sttUsageTracker.deepgramStreamSecondsSinceReset = (sttUsageTracker.deepgramStreamSecondsSinceReset || 0) + durationSec;
  sttUsageTracker.deepgramTodayRequests = sttUsageTracker.deepgramRequestsSinceReset;
  sttUsageTracker.deepgramStreamSecondsToday = sttUsageTracker.deepgramStreamSecondsSinceReset;
...
// Helper: Check if 4-byte sequence at offset is a valid MPEG Audio (MP3/MP2) frame header
function isValidMpegHeader(buffer: Buffer, offset: number): boolean {
  ...
  // Sync word: 11 bits of 1s (0xFF followed by top 3 bits 111)
  if (b0 !== 0xFF || (b1 & 0xE0) !== 0xE0) return false;

  // MPEG audio version: bits 4-3 of b1 (00 = 2.5, 01 = reserved, 10 = 2, 11 = 1)
  const version = (b1 >> 3) & 0x03;
  if (version === 1) return false; // reserved/invalid

  // Layer: bits 2-1 of b1 (00 = reserved, 01 = Layer III, 10 = Layer II, 11 = Layer I)
  const layer = (b1 >> 1) & 0x03;
  if (layer === 0) return false; // reserved/invalid

  // Bitrate index: bits 7-4 of b2 (0000 = free, 1111 = bad/invalid)
  const bitrateIdx = (b2 >> 4) & 0x0F;
  if (bitrateIdx === 0x0F) return false;

  // Sampling rate index: bits 3-2 of b2 (11 = reserved/invalid)
  const sampleRateIdx = (b2 >> 2) & 0x03;
  if (sampleRateIdx === 3) return false;

  // Emphasis: bits 1-0 of b3 (10 = reserved/invalid)
...
// Helper: Check if 7-byte sequence at offset is a valid AAC ADTS frame header
function isValidAacAdtsHeader(buffer: Buffer, offset: number): boolean {
  ...
  if (b0 !== 0xFF || (b1 & 0xF6) !== 0xF0) return false;

  // Sampling frequency index: bits 5-2 of b2 (0-11 are valid, 12-15 reserved/invalid)
  const sampleFreqIdx = (b2 >> 2) & 0x0F;
  if (sampleFreqIdx > 11) return false;

  return true;
}

// Helper: Find verified frame sync header (AAC ADTS 0xFFF or MP3 0xFFE) to avoid leading partial-frame corruption
function alignAudioBuffer(buffer: Buffer): Buffer {
  if (!buffer || buffer.length < 512) return buffer;
  // If buffer already starts with a valid header, use directly
  if (isValidMpegHeader(buffer, 0) || isValidAacAdtsHeader(buffer, 0)) {
    return buffer;
  }
  // Search within the first 8KB for a strictly validated header
  const maxSearch = Math.min(buffer.length - 7, 8192);
  for (let i = 0; i < maxSearch; i++) {
    if (buffer[i] === 0xFF) {
      if (isValidAacAdtsHeader(buffer, i) || isValidMpegHeader(buffer, i)) {
...
// Internal worker: spawn ffmpeg to convert buffer to 16kHz mono WAV
function runFfmpegConversion(buffer: Buffer): Promise<Buffer> {
  return new Promise((resolve) => {
    ...
    ff.on('close', (code: number) => {
      ...
      if (out.length > 2000) {
        ...
        // Return empty buffer gracefully without throwing
        resolve(Buffer.alloc(0));
      }
    });
    ff.on('error', () => {
...
async function convertToWav(inputBuffer: Buffer): Promise<Buffer> {
  ...
  // Attempt 1: Try with frame-aligned buffer
  const alignedBuffer = alignAudioBuffer(inputBuffer);
  const outAligned = await runFfmpegConversion(alignedBuffer);
  if (outAligned.length >= 2000) {
    return outAligned;
  }

  // Attempt 2: If aligned buffer produced insufficient output, fallback to raw inputBuffer (ffmpeg's native stream demuxer)
  if (alignedBuffer !== inputBuffer) {
    const outRaw = await runFfmpegConversion(inputBuffer);
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      ...
      try {
        ...
        if (res.status === 429) {
          ...
          // Check if BOTH models are daily quota exhausted
          const bothDailyQuotaExhausted = groqTurboDailyQuotaExhausted && groqV3DailyQuotaExhausted;
          if (bothDailyQuotaExhausted) {
            groqRateLimitedUntil = Date.now() + backoffMs;
            console.warn(`[Groq Daily Quota Exhausted ⏳] Both Turbo and V3 daily quotas reached (4,000 RPD).`);
            if (DEEPGRAM_TOKEN && !deepgramWs && triggerDeepgramFallback) {
              console.info('[STT Fallback] Activating Deepgram WebSocket fallback because both Groq quotas are exhausted...');
              triggerDeepgramFallback();
            }
          } else {
            console.info(`[Groq Fast Failover ⚡] ${model} hit momentary 429 limit. Failing over to ${secondaryModel}...`);
          }
  ...
  res.json({
    ...
    sttUsage: {
      ...
      deepgram: {
        ...
        rpm: deepgramRpm,
        resetAt: sttUsageTracker.deepgramResetAt || new Date().toISOString(),
        streamSecondsSinceReset: Math.round(sttUsageTracker.deepgramStreamSecondsSinceReset || 0),
        requestsSinceReset: sttUsageTracker.deepgramRequestsSinceReset || 0,
        streamSecondsToday: Math.round(sttUsageTracker.deepgramStreamSecondsSinceReset || 0),
        requestsTodayUtc: sttUsageTracker.deepgramRequestsSinceReset || 0,
        estimatedCostUsd: Number((((sttUsageTracker.deepgramStreamSecondsSinceReset || 0) / 60) * 0.0043).toFixed(4)),
        estimatedCostNtd: Math.round((((sttUsageTracker.deepgramStreamSecondsSinceReset || 0) / 60) * 0.0043) * 32.5),
...
app.post('/api/sync-stt-usage', express.json(), (req, res) => {
  try {
    ...
    if (maxTurbo < 2000) groqTurboDailyQuotaExhausted = false;
    if (maxV3 < 2000) groqV3DailyQuotaExhausted = false;
...
app.post('/api/reset-deepgram', (req, res) => {
  sttUsageTracker.deepgramRequestsSinceReset = 0;
  sttUsageTracker.deepgramStreamSecondsSinceReset = 0;
  sttUsageTracker.deepgramTodayRequests = 0;
  sttUsageTracker.deepgramStreamSecondsToday = 0;
  sttUsageTracker.deepgramResetAt = new Date().toISOString();
  saveSttUsageTracker();
  res.json({
    ok: true,
    status: 'reset',
    resetAt: sttUsageTracker.deepgramResetAt,
    streamSeconds: 0,
    costUsd: 0,
    costNtd: 0,
  });
...
function normalizeAllCapitalizedSpeech(text: string): string {
  ...
  return text.replace(/[a-zA-Z]+(?:'[a-zA-Z]+)?|[^a-zA-Z]+/g, (token) => {
    if (/^[a-zA-Z]+(?:'[a-zA-Z]+)?$/.test(token)) {
      ...
      // Conversational turn starters
      if (/^(?:hi|hello|hey|yes|no|well|so)\b/i.test(token)) {
...
/**
 * Ensures that sentences and dialogue turns are properly capitalized for language learning.
 * 1. Capitalizes the very first letter of the string.
 * 2. Capitalizes letters following sentence terminators (. ? ! ;).
 * 3. Capitalizes standalone 'I' and contractions (I'm, I've, I'll, I'd).
 * 4. Capitalizes recognized acronyms (NPR, BBC, USA, AI, etc.).
 * 5. Capitalizes conversational turn starters following punctuation or boundaries.
 */
function formatSentenceCase(text: string): string {
  ...
  // 2. Capitalize after sentence terminators (. ? ! ;) followed by space
  s = s.replace(/([.?!;]\s+)([a-z])/g, (_, punct, letter) => punct + letter.toUpperCase());

  // 3. Capitalize standalone 'i' and standard contractions
  s = s.replace(/\b(i)('m|'ve|'ll|'d)?\b/g, (_, iChar, suffix) => 'I' + (suffix || ''));

  // 4. Preserve / restore known acronyms
  KNOWN_UPPERCASE_ACRONYMS.forEach((acronym) => {
    const regex = new RegExp(`\\b${acronym}\\b`, 'gi');
    s = s.replace(regex, acronym);
  });

  // 5. Ensure common dialogue/conversation starters after punctuation are capitalized
  s = s.replace(/([,;:]\s+)(hi|hello|hey|yes|no|well|so|now|actually|certainly|indeed)\b/gi, (_, punct, word) => {
    return punct + word.charAt(0).toUpperCase() + word.slice(1);
  });
...
function splitIntoLearningSentences(text: string): string[] {
  ...
  // Refine each sentence: if excessively long (> 14 words), split at clause boundary
  const learningSentences: string[] = [];
  for (const s of rawSentences) {
    const words = s.split(/\s+/).filter(Boolean);
    if (words.length <= 14) {
      learningSentences.push(formatSentenceCase(s));
      continue;
    }

    const clauseTokens = s.split(/([,—:]\s+|\s+—\s+|\s+-\s+|(?<=\s)(?:so tell us|tell us|what's|how is|yes it's|no it's)\b)/gi);
    let clauseAcc = '';
    for (let j = 0; j < clauseTokens.length; j++) {
      const part = clauseTokens[j];
      clauseAcc += part;
      const curWords = clauseAcc.split(/\s+/).filter(Boolean);
      if (curWords.length >= 6 && j < clauseTokens.length - 1) {
        learningSentences.push(formatSentenceCase(clauseAcc.trim()));
        clauseAcc = '';
      }
    }
...
function stopBackendStreaming() {
  ...
  groqLastContext = '';
  lastFlushedText = '';
  recentEmittedServerSentences.length = 0;
...
function mergeTranscriptChunk(pending: string, chunk: string): string {
  ...
  if (pWords.length === 0) return c;
  if (cWords.length === 0) return p;

  const normP = pWords.map((w) => w.toLowerCase().replace(/[^a-z0-9]/g, ''));
  const normC = cWords.map((w) => w.toLowerCase().replace(/[^a-z0-9]/g, ''));
  ...
  for (let k = maxOverlap; k >= 2; k--) {
    const pendingSuffix = normP.slice(-k);
    const chunkPrefix = normC.slice(0, k);
    if (pendingSuffix.every((word, idx) => word === chunkPrefix[idx])) {
      if (k === cWords.length) {
        return p;
      }
      const nonOverlappingWords = cWords.slice(k).join(' ');
      return `${p} ${nonOverlappingWords}`;
    }
  }

  // Single word overlap check
  if (maxOverlap >= 1 && normP[normP.length - 1] === normC[0] && normP[normP.length - 1].length >= 4) {
    if (cWords.length === 1) {
      return p;
    }
    const nonOverlappingWords = cWords.slice(1).join(' ');
    return `${p} ${nonOverlappingWords}`;
  }
...
function handleSpeechTranscriptChunk(transcript: string) {
  ...
  const mergedText = mergeTranscriptChunk(currentPending, cleanChunk);

  if (!pendingTranscriptBuffer) {
    bufferStartTime = Date.now();
  }
  pendingTranscriptBuffer = mergedText;
  ...
  radioReq = requester.get(requestOptions, (radioRes) => {
    ...
    radioRes.on('data', (chunk: Buffer) => {
      ...
      if (GROQ_TOKEN) {
        ...
        const now = Date.now();
        const activeModel = getActiveGroqModel();
        const turboDailyExhausted = groqTurboDailyQuotaExhausted || (sttUsageTracker?.groqModelsUsage?.[MODEL_TURBO]?.requestsToday || 0) >= 2000;
        const v3DailyExhausted = groqV3DailyQuotaExhausted || (sttUsageTracker?.groqModelsUsage?.[MODEL_V3]?.requestsToday || 0) >= 2000;
        const bothDailyExhausted = turboDailyExhausted && v3DailyExhausted;
        const turboInCooldown = now < groqTurboCooldownUntil;
        const v3InCooldown = now < groqV3CooldownUntil;
        const isAnyModelReady = (!turboDailyExhausted && !turboInCooldown) || (!v3DailyExhausted && !v3InCooldown);
        ...
        // ONLY if BOTH Groq models are verified DAILY QUOTA EXHAUSTED (or >= 10 sustained errors), activate Deepgram fallback
        if ((bothDailyExhausted || groqConsecutiveErrors >= 10) && DEEPGRAM_TOKEN && !deepgramWs) {
          console.log('[STT Failover] All Groq daily quotas exhausted, activating Deepgram Nova-2 fallback...');
          initDeepgramWs();
        } else if (!bothDailyExhausted && groqConsecutiveErrors < 10 && deepgramWs) {
          // Put Deepgram back to standby immediately to guarantee $0 cost!
          console.log(`[STT Recovery] Groq daily quota available (${activeModel}). Putting Deepgram back to 0 RPM standby...`);
          try {
        ...
        // When accumulator has accumulated ~3.8-4.0s of audio and rate limit conditions allow
        const timeSinceLastGroq = now - lastGroqRequestTime;
        const hasEnoughAudio = (groqAudioAccumulator.length >= GROQ_BUFFER_THRESHOLD) ||
                               (timeSinceLastGroq >= 3800 && groqAudioAccumulator.length >= 28000);
        if (hasEnoughAudio && !isGroqTranscribing) {
          if (isCooldownOver && isIntervalElapsed) {
            ...
            // Retain ~35% audio overlap (~24KB max) to prevent boundary syllable cutoff
            const overlapLength = Math.min(Math.floor(bufferToTranscribe.length * 0.35), 24000);
            groqAudioAccumulator = groqAudioAccumulator.slice(Math.max(0, groqAudioAccumulator.length - overlapLength));
            ...
            (async () => {
              try {
                ...
                if ((bothDailyExhausted || groqConsecutiveErrors >= 10) && DEEPGRAM_TOKEN && !deepgramWs) {
                  console.warn('[STT Fallback] Sustained Groq failures/exhaustion, activating Deepgram WebSocket fallback...');
                  initDeepgramWs();
                }
              } finally {
                isGroqTranscribing = false;
