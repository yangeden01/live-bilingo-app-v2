...
export function useSubtitleSync({
  ...
  useEffect(() => {
    ...
    if (playbackStatus !== 'PLAYING' && playbackStatus !== 'BUFFERING') {
      ...
      if (onInterimSubtitle) {
        onInterimSubtitle(null);
      }
    }
  }, [playbackStatus, onInterimSubtitle]);
  ...
  const ingestSubtitle = useCallback(
    (item: SubtitleItem) => {
      ...
      // Strict Station Isolation: Reject any subtitle lacking stationUrl or belonging to another station
      const currentStationUrl = activeStationRef.current?.streamUrl;
      if (!item.stationUrl || !isStationUrlMatch(item.stationUrl, currentStationUrl)) {
        return;
      }
      ...
      // When radio playback is not active (paused or idle), do not advance live subtitles
  ...
  useEffect(() => {
    (window as any).handleNativeSubtitle = (sub: any) => {
      ...
      if (sub && sub.id && sub.english) {
        ...
        const item = { ...sub, traditionalChinese: zh, isNative: true };
        ingestSubtitle(item);
        window.dispatchEvent(new CustomEvent('new-subtitle', { detail: item }));
      }
    };
  ...
  }, [activeStation.streamUrl, activeStation.name, playbackStatus, isActive]);

  // 3. Primary SSE Stream with REST polling fallback & Autonomous Recovery Watchdog
  useEffect(() => {
    if (!isActive) return;

    // Clear deduplication cache and decoupled queue when active station changes
    lastProcessedSubtitleIdRef.current.clear();
    if (timeAlignerRef.current) {
      timeAlignerRef.current.clear();
    }
    ...
    const pollSubtitles = async (forceRecent = false) => {
      ...
      try {
        ...
        const encodedStation = encodeURIComponent(activeStation.streamUrl);
        const url = getApiUrl(`/api/live-subtitles?since=${queryTimestamp}&stationUrl=${encodedStation}`);
    ...
    const connectWebSocket = () => {
      ...
      try {
        const encodedStation = encodeURIComponent(activeStation.streamUrl);
        const wsUrl = getWsApiUrl(`/api/subtitles-ws?stationUrl=${encodedStation}`);
    ...
    const connectSSE = () => {
      ...
      try {
        const encodedStation = encodeURIComponent(activeStation.streamUrl);
        es = new EventSource(getApiUrl(`/api/live-subtitles-stream?stationUrl=${encodedStation}`));
    ...
    const triggerComprehensiveSubtitleRecovery = () => {
      const isCurrentlyPlaying = playbackStatusRef.current === 'PLAYING' || playbackStatusRef.current === 'BUFFERING';
      if (!isCurrentlyPlaying) {
        console.log('[Subtitle Sync] Radio is not playing (status:', playbackStatusRef.current, '). Skipping subtitle recovery & STT activation.');
        return;
      }
    ...
    const handleAppForeground = (reason: string = 'foreground_state') => {
      ...
      // Notify backend that user returned to foreground and wake up STT immediately
      safeApiFetch('/api/subtitle-stream-state', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      // Notify backend to make sure station audio capture is running
      safeApiFetch('/api/notify-station-playing', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      // Reconnect polling interval if not active
      if (!pollInterval) {
        pollInterval = setInterval(pollSubtitles, 2500);
      }
    ...
    const handleAppWakeupEvent = () => {
      handleAppForeground('wakeup_event');
    };
    ...
    window.addEventListener('app-foreground-wakeup-subtitles', handleAppWakeupEvent);
    ...
    return () => {
      ...
      window.removeEventListener('app-foreground-wakeup-subtitles', handleAppWakeupEvent);
