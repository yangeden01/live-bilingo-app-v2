...
class RadioStreamSttManager(
    ...
    fun getUsageStatsJson(): String {
        ...
        val rpm = synchronized(connectionTimestamps) {
            val cutoff = now - 60000L
            connectionTimestamps.removeAll { it < cutoff }
            connectionTimestamps.size
        }
        val json = JSONObject()
        json.put("engine", "Deepgram Nova-2 (Native)")
        json.put("rpm", rpm)
        json.put("requestsTodayUtc", todayRequestsCount)
        json.put("utcDate", currentUtcDay)
        json.put("active", webSocket != null)
