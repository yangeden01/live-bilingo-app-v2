...
export class DecoupledTimeAligner {
  ...
  public enqueue(item: SubtitleItem, audioCurrentTime?: number) {
    ...
    let releaseAt = now;
    if (item.scheduledReleaseTime && typeof item.scheduledReleaseTime === 'number') {
      releaseAt = Math.max(now, item.scheduledReleaseTime);
    } else {
    ...
    // Immediately evaluate queue if scheduled release time has already passed
    if (releaseAt <= now) {
      this.tick();
    }
  }
  ...
  /**
   * Fast release all pending subtitles (e.g. on manual seek)
   */
  public flushAll() {
