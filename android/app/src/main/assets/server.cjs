...
function formatRelativeTimeZh(publishedAt) {
  const diffMs = Math.max(0, Date.now() - new Date(publishedAt).getTime());
  const mins = Math.floor(diffMs / (60 * 1e3));
  if (mins < 1) return "\u525B\u525B";
...
function categorizeNewsVideo(title, channelTitle, defaultCat = "World") {
  ...
  if (channelTitle.includes("The Verge") || /\b(ai|artificial intelligence|tech|technology|nvidia|openai|semiconductor|cyber|google|apple|microsoft|gadget|robot)\b/i.test(lower)) {
    return "Technology";
  }
  if (channelTitle.includes("Bloomberg") || channelTitle.includes("CNBC") || channelTitle.includes("Wall Street Journal") || channelTitle.includes("Financial Times") || /\b(economy|inflation|wall street|stock|market|fed|interest rate|trade|tariff|ceo|bank|investor)\b/i.test(lower)) {
    return "Business";
  }
  if (/\b(biden|trump|congress|senate|white house|supreme court|fbi|pentagon|us election|election|harris|republican|democrat)\b/i.test(lower)) {
    return "US";
  }
  return defaultCat;
}
function isNewsContent(title, channelTitle) {
  const lower = title.toLowerCase();
  if (lower.includes("#shorts") || lower.includes("#short") || lower.includes(" shorts") || lower.includes("shorts ")) {
    return false;
  }
  if (/^(?:live:|\(live\)|\[live\]|\|\s*live)/i.test(title.trim()) || /\b(?:live stream|streaming live|watch live)\b/i.test(lower)) {
    return false;
  }
...
function deduplicateNewsVideos(videos) {
  ...
  for (const v of videos) {
    const tokens = v.title.toLowerCase().replace(/[^\w\s]/g, "").split(/\s+/).filter((t) => t.length > 3);
    const tokenSet = new Set(tokens);
    let isDuplicate = false;
    for (const prevTitle of seenTitles) {
      const prevTokens = prevTitle.toLowerCase().replace(/[^\w\s]/g, "").split(/\s+/).filter((t) => t.length > 3);
...
async function discoverLiveEnglishNews(forceRefresh = false) {
  const now = Date.now();
  const oneWeekAgoMs = now - 7 * 24 * 60 * 60 * 1e3;
  const publishedAfterIso = new Date(oneWeekAgoMs).toISOString();
  if (!forceRefresh && newsCache && now - newsCache.timestamp < CACHE_TTL_MS && newsCache.videos.length > 0) {
    return {
      success: true,
      cached: true,
      ...
      REPUTABLE_NEWS_CHANNELS.map(async (ch) => {
        ...
        for (const entry of entries) {
          ...
          items.push({
            ...
            // Default fallback duration estimate
            durationSeconds: 300,
            thumbnailUrl: thumb,
            hasCaptions: true,
  ...
  if (apiKey && candidateVideos.length > 0) {
    try {
      ...
      const validDetailsMap = /* @__PURE__ */ new Map();
...
async function enrichNewsVideosWithDetailsAndTranslation(videos) {
  ...
  if (geminiKey && titlesToTranslate.length > 0) {
    try {
      ...
      const prompt = "Translate the following English news titles into authentic, fluent Traditional Chinese (Taiwan, \u7E41\u9AD4\u4E2D\u6587\u53F0\u7063\u7FD2\u6163\u7528\u8A9E). Return ONLY a valid JSON array of strings corresponding to each title in order, with no explanation or markdown tags:\n" + JSON.stringify(titlesToTranslate);
      let resp;
      try {
...
function getActiveGroqModel(preferred, stationUrl) {
  ...
  const turboExhausted = groqTurboDailyQuotaExhausted || turboUsage && turboUsage.requestsToday >= 2e3;
  const turboInCooldown = now < groqTurboCooldownUntil;
  const v3Usage = sttUsageTracker?.groqModelsUsage?.[MODEL_V3];
  const v3Exhausted = groqV3DailyQuotaExhausted || v3Usage && v3Usage.requestsToday >= 2e3;
...
var sttUsageTracker = {
  ...
  deepgramResetAt: (/* @__PURE__ */ new Date()).toISOString(),
          ...
          for (const [m, info] of Object.entries(data.groqModelsUsage)) {
            if (sttUsageTracker.groqModelsUsage[m]) {
              ...
              if (info.remainingRequests) {
                ...
                const isDailyQuotaReset = info.resetRequests && /h|d/i.test(info.resetRequests) || (info.requestsToday || 0) >= 2e3;
...
function recordGroqRequest(headers, extra) {
  ...
  if (headers) {
    ...
    const isResetDaily = (resetReq ? /h|d/i.test(resetReq) : false) || sttUsageTracker.groqModelsUsage[currentModel].requestsToday >= 2e3;
    if (remReq !== null) {
...
function isValidAacAdtsHeader(buffer, offset) {
  if (offset + 7 > buffer.length) return false;
  const b0 = buffer[offset];
  const b1 = buffer[offset + 1];
  const b2 = buffer[offset + 2];
  if (b0 !== 255 || (b1 & 246) !== 240) return false;
  const sampleFreqIdx = b2 >> 2 & 15;
...
function runFfmpegConversion(buffer) {
  return new Promise((resolve) => {
  ...
  res.json({
    ...
    sttUsage: {
      ...
      deepgram: {
        ...
        rpm: deepgramRpm,
        resetAt: sttUsageTracker.deepgramResetAt || (/* @__PURE__ */ new Date()).toISOString(),
        ...
        estimatedCostUsd: Number(((sttUsageTracker.deepgramStreamSecondsSinceReset || 0) / 60 * 43e-4).toFixed(4)),
        estimatedCostNtd: Math.round((sttUsageTracker.deepgramStreamSecondsSinceReset || 0) / 60 * 43e-4 * 32.5),
...
app.post("/api/sync-stt-usage", import_express.default.json(), (req, res) => {
  try {
    ...
    if (maxTurbo < 2e3) groqTurboDailyQuotaExhausted = false;
    if (maxV3 < 2e3) groqV3DailyQuotaExhausted = false;
    saveSttUsageTracker();
    res.json({
      ok: true,
...
app.post("/api/reset-deepgram", (req, res) => {
  ...
  sttUsageTracker.deepgramTodayRequests = 0;
  sttUsageTracker.deepgramStreamSecondsToday = 0;
  sttUsageTracker.deepgramResetAt = (/* @__PURE__ */ new Date()).toISOString();
  saveSttUsageTracker();
  res.json({
    ok: true,
    status: "reset",
...
function mergeTranscriptChunk(pending, chunk) {
  ...
  radioReq = requester.get(requestOptions, (radioRes) => {
    ...
    radioRes.on("data", (chunk) => {
      ...
      if (GROQ_TOKEN) {
        ...
        const now = Date.now();
        const activeModel = getActiveGroqModel();
        const turboDailyExhausted = groqTurboDailyQuotaExhausted || (sttUsageTracker?.groqModelsUsage?.[MODEL_TURBO]?.requestsToday || 0) >= 2e3;
        const v3DailyExhausted = groqV3DailyQuotaExhausted || (sttUsageTracker?.groqModelsUsage?.[MODEL_V3]?.requestsToday || 0) >= 2e3;
        ...
        if ((bothDailyExhausted || groqConsecutiveErrors >= 10) && DEEPGRAM_TOKEN && !deepgramWs) {
          ...
          try {
            if (deepgramWs.readyState === import_ws.default.OPEN || deepgramWs.readyState === import_ws.default.CONNECTING) {
              deepgramWs.close();
            }
          } catch (_) {
          }
        ...
        const hasEnoughAudio = groqAudioAccumulator.length >= GROQ_BUFFER_THRESHOLD || timeSinceLastGroq >= 3800 && groqAudioAccumulator.length >= 28e3;
        if (hasEnoughAudio && !isGroqTranscribing) {
          if (isCooldownOver && isIntervalElapsed) {
            ...
            const overlapLength = Math.min(Math.floor(bufferToTranscribe.length * 0.35), 24e3);
