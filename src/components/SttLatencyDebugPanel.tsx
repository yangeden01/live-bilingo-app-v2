...
export function SttLatencyDebugPanel({ sttConnected, activeStationName }: SttLatencyDebugPanelProps) {
  ...
  useEffect(() => {
    const handleSubtitleEvent = (e: any) => {
      const now = Date.now();
      const cutoff = now - 60000;

      // 避免同一則字幕在 new-subtitle 與 native-subtitle 雙通道重複觸發
      const subId = e?.detail?.id || (e?.detail?.english ? `${e.detail.english.slice(0, 30)}_${e.detail.timestamp || ''}` : '');
      if (subId) {
        if (processedEventIdsRef.current.has(subId)) return;
        processedEventIdsRef.current.add(subId);
        if (processedEventIdsRef.current.size > 200) {
          const arr = Array.from(processedEventIdsRef.current);
          processedEventIdsRef.current = new Set(arr.slice(arr.length - 100));
        }
      }
  ...
  // Deepgram 手動重置與累積費用追蹤（用戶明確要求：非每日重置，手動按鈕重置並記錄時間）
  const [isResettingDeepgram, setIsResettingDeepgram] = useState(false);
  const [deepgramResetTimestamp, setDeepgramResetTimestamp] = useState<string>(() => {
    try {
      return localStorage.getItem('bilingo_dg_reset_at') || '';
    } catch (_) {
      return '';
    }
  });
  ...
  // 同步後端持久化記錄的重置時間
  useEffect(() => {
    if (deepgramUsage?.resetAt) {
      const serverResetAt = String(deepgramUsage.resetAt);
      if (serverResetAt && serverResetAt !== deepgramResetTimestamp) {
        setDeepgramResetTimestamp(serverResetAt);
        try {
          localStorage.setItem('bilingo_dg_reset_at', serverResetAt);
        } catch (_) {}
      }
    }
  }, [deepgramUsage?.resetAt]);

  const handleResetDeepgram = async () => {
    if (isResettingDeepgram) return;
    setIsResettingDeepgram(true);
    try {
      const res = await safeApiFetch<any>('/api/reset-deepgram', {
        method: 'POST',
      });
      const data = res.data;
      const newResetAt = data?.resetAt || new Date().toISOString();
      setDeepgramResetTimestamp(newResetAt);
      try {
        localStorage.setItem('bilingo_dg_reset_at', newResetAt);
        localStorage.setItem('bilingo_dg_sec_manual', '0');
      } catch (_) {}
      setSttStatus((prev: any) => {
        if (!prev) return prev;
        return {
          ...
          sttUsage: {
            ...(prev.sttUsage || {}),
            deepgram: {
              ...(prev.sttUsage?.deepgram || {}),
              streamSecondsSinceReset: 0,
              streamSecondsToday: 0,
      ...
      console.warn('Failed to reset deepgram usage:', e);
    } finally {
      setIsResettingDeepgram(false);
    }
  };

  const cachedDgSeconds = (() => {
    try {
      return Number(localStorage.getItem('bilingo_dg_sec_manual') || '0');
    } catch (_) {
      return 0;
    }
  })();
  ...
  const rawDgSeconds = Number(
    deepgramUsage?.streamSecondsSinceReset ??
      deepgramUsage?.streamSecondsToday ??
      (Number(deepgramUsage?.requestsSinceReset ?? deepgramUsage?.requestsTodayUtc ?? 0) * 3.5)
  );
  const deepgramSeconds = Math.max(cachedDgSeconds, rawDgSeconds);

  if (deepgramSeconds > cachedDgSeconds) {
    try {
      localStorage.setItem('bilingo_dg_sec_manual', String(deepgramSeconds));
    } catch (_) {}
  }
  ...
  const formattedDgResetTime = (() => {
    const rawTime = deepgramResetTimestamp || deepgramUsage?.resetAt;
    if (!rawTime) return '尚未手動重置（自初次安裝起累計）';
    try {
              ...
              >
                <div className="flex items-center justify-between mb-1.5 gap-1.5 flex-wrap">
                  ...
                  <div className="flex items-center gap-1.5">
                    {/* Deepgram 手動重置計費按鈕 */}
                    <button
                      id="btn-reset-deepgram"
                      type="button"
                      onClick={handleResetDeepgram}
                      disabled={isResettingDeepgram}
                      title="手動重置 Deepgram 計費時長與費用統計，並記錄本次重置時間"
                      className="px-2 py-0.5 rounded bg-amber-900/70 hover:bg-amber-800 text-amber-200 border border-amber-600/80 text-[10px] font-medium flex items-center gap-1 transition-all active:scale-95 cursor-pointer disabled:opacity-50 shadow-sm"
                    >
                      <RotateCcw className={`w-2.5 h-2.5 ${isResettingDeepgram ? 'animate-spin' : ''}`} />
                      <span>{isResettingDeepgram ? '重置中...' : '重置計費'}</span>
                    </button>
                    <span
                      className={`px-1.5 py-0.5 rounded text-[10px] font-sans font-medium ${
                        isDeepgramActive
                ...
                <div className="grid grid-cols-2 gap-2 mb-1.5 font-mono">
                  <div className="p-1.5 rounded bg-slate-900/90 border border-slate-800">
                    <div className="text-[9px] text-slate-400 font-sans">累積串流時長</div>
                    <div className="text-xs font-bold text-slate-200">
                      {deepgramDurationText}
                    </div>
                    <div className="text-[8px] text-slate-500 font-sans truncate" title={`自 ${formattedDgResetTime} 起算`}>
                      自 {formattedDgResetTime}
                    </div>
                  </div>
                  <div className="p-1.5 rounded bg-slate-900/90 border border-slate-800">
                    <div className="text-[9px] text-slate-400 font-sans">期間預估花費</div>
                    <div className={`text-xs font-bold ${isDeepgramActive ? 'text-amber-300' : 'text-emerald-400'}`}>
                      {deepgramCostText}
                    </div>
                    <div className="text-[8px] text-slate-500 font-sans">手動週期 • $0.0043/分</div>
                  </div>
                </div>

                {/* 重置時間記錄條 */}
                <div className="p-1.5 mb-1.5 rounded bg-slate-900/80 border border-slate-800/80 flex items-center justify-between text-[9px] text-slate-400 font-sans">
                  <div className="flex items-center gap-1.5 truncate">
                    <Clock className="w-3 h-3 text-amber-400 shrink-0" />
                    <span className="truncate">
                      上次重置時間: <strong className="text-amber-200 font-mono font-semibold">{formattedDgResetTime}</strong>
                    </span>
                  </div>
                  <span className="text-[8px] text-slate-500 shrink-0 pl-1">
                    手動歸零計費
                  </span>
                </div>
                ...
                <div className="text-[9px] text-slate-400 font-sans">
                  {bothGroqExhausted ? (
                    ...
                    <span>🛡️ 當 Groq 雙模型合計 4,000 次滿載時，才無縫啟動接棒。平常 0 耗損不扣款，點擊「重置計費」可精確掌握各時段 Deepgram 花費。</span>
                  )}
                </div>
              </div>
            </div>
