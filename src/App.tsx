...
import { Radio, Code2, Smartphone, Cpu, CheckCircle2, Sparkles, Volume2, ShieldCheck, Download, ListMusic, BookOpen, RefreshCw, Copy, Play, Pause, Sun, Moon, Bell, Power, ArrowUp } from 'lucide-react';
...
export default function App() {
          ...
          if (cleanedList.length > 0) {
            ...
            dedupedList.forEach((item: SubtitleItem, revIdx: number) => {
              ...
              if ((chronIdx + 1) >= 15 && (chronIdx + 1) % 15 === 0) {
                ...
                item.attachedAdIndex = (Math.floor((chronIdx + 1) / 15) - 1) % 5;
              } else {
                item.hasAttachedAd = false;
                delete (item as any).attachedAdIndex;
  ...
  const handleNewSubtitle = useCallback((item: SubtitleItem) => {
    ...
    setSubtitles((prev) => {
      ...
      if (existingIndex >= 0) {
        ...
        // Check if this new item should permanently attach an in-feed ad (every 15th paragraph: at 15, 30, 45...)
        const newTotalCount = prev.length + 1;
        if (newTotalCount >= 15 && newTotalCount % 15 === 0) {
          cleanItem.hasAttachedAd = true;
  ...
  // Smart Reading Mode Magnetic Detent Hook
  ...
  return (
    <div className={`min-h-screen font-sans selection:bg-blue-600 selection:text-white pb-12 transition-colors duration-200 ${appBgClass}`}>
      ...
      <AnimatePresence>
        {showBackToReadingFab && activeTab === 'app' && (
          <motion.button
      ...
      {/* Bottom Sticky AdMob Banner Ad */}
      <div className="sticky bottom-0 z-30 w-full bg-slate-900/95 backdrop-blur border-t border-slate-700/80 shadow-[0_-4px_16px_rgba(0,0,0,0.3)] pb-[env(safe-area-inset-bottom)]">
