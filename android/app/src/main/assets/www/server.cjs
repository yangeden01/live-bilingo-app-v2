...
function categorizeNewsVideo(title, channelTitle, defaultCat = "World") {
  ...
  if (channelTitle.includes("Bloomberg") || channelTitle.includes("CNBC") || /\b(economy|inflation|wall street|stock|market|fed|interest rate|trade|tariff|ceo|bank|investor)\b/i.test(lower)) {
    return "Business";
  }
...
async function discoverLiveEnglishNews(forceRefresh = false) {
  const now = Date.now();
  const fortyEightHoursAgoMs = now - 48 * 60 * 60 * 1e3;
  ...
  if (apiKey && candidateVideos.length < 15) {
    try {
      ...
      if (searchRes.ok) {
        ...
        const videoIds = items.map((i) => i?.id?.videoId).filter((id) => typeof id === "string" && id.length === 11);
            ...
            for (const item of videosData.items || []) {
              ...
              if (!candidateVideos.some((cv) => cv.videoId === vId)) {
                candidateVideos.push({
                  ...
                  thumbnailUrl: snippet.thumbnails?.high?.url || snippet.thumbnails?.medium?.url || `https://i.ytimg.com/vi/${vId}/hqdefault.jpg`,
                  hasCaptions: true,
                  captionBadge: "CC \u82F1\u6587\u5B57\u5E55",
                  category: cat
                });
