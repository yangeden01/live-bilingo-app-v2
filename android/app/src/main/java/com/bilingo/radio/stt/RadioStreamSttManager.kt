...
class RadioStreamSttManager(
    ...
    private val groqProcessingMutex = Mutex()
    @Volatile private var nextChunkSequence = 1L
    ...
    private fun getActiveGroqModel(stationUrl: String? = null): String {
        ...
        // Rule 1: Always prioritize Groq Turbo as primary free tier model
        return "whisper-large-v3-turbo"
    }
    ...
    private fun recordNativeGroqRequest(
        ...
        if (model == "whisper-large-v3") {
            ...
            if (status == 200 && groqV3RequestsToday < 2000) {
                groqV3Exhausted = false
            }
        } else {
            ...
            if (status == 200 && groqTurboRequestsToday < 2000) {
                groqTurboExhausted = false
            }
        }
        groqTotalRequestsToday = groqTurboRequestsToday + groqV3RequestsToday
        ...
        if (status == 429) {
            val backoffMs = 8000L
            // Groq header x-ratelimit-remaining-requests is for RPM (0-20), NOT daily quota!
            // Only consider exhausted if reset string is in hours ("h") or days ("d"), or actual requests today >= 2000
            val isDailyQuota = (!reset.isNullOrBlank() && (reset.contains("h") || reset.contains("d"))) ||
                (model == "whisper-large-v3-turbo" && groqTurboRequestsToday >= 2000) ||
                (model == "whisper-large-v3" && groqV3RequestsToday >= 2000)
    ...
    private fun resolveTargetStreamUrl(rawUrl: String): String {
        ...
        if (target.contains("abc.streamguys1.com", ignoreCase = true) || target.contains("icecast.audio", ignoreCase = true)) {
        ...
        if (target.contains("NOVA_SKYNEWSAAC.aac", ignoreCase = true)) {
        ...
        if (target.contains("foxnewsradioaac", ignoreCase = true)) {
        ...
        if (target.contains("stream.revma.ihrhls.com/zc6043", ignoreCase = true)) {
        ...
        try {
            ...
            while (isRunning && scope.isActive && sessionId == currentSessionId) {
                ...
                if (isDeepgramBackupActive) {
                    ...
                    val turboReady = !groqTurboExhausted && curNow >= groqTurboCooldownUntil
                    val v3Ready = !groqV3Exhausted && curNow >= groqV3CooldownUntil
                    if (turboReady || v3Ready) {
                        android.util.Log.i("RadioStreamSttManager", "[STT Recovery] Groq available (Turbo=$turboReady, V3=$v3Ready). Recovering from Deepgram to Groq Free Primary!")
                ...
                if (isDeepgramBackupActive && webSocket != null) {
                    ...
                    // Slice ~3.8-4.0s of audio for ultra-tight live sync (~15.7 RPM, well within 20/40 RPM limit)
                    // Subtitles appear with 0 lag or ~0.5s ahead of broadcast sound
                    val currentAccumulated = audioAccumulator.size()
                    if ((currentAccumulated >= 58000 && timeSinceLast >= 3500L) || (timeSinceLast >= 3800L && currentAccumulated >= 28000)) {
                        val fullBytes = audioAccumulator.toByteArray()
                        audioAccumulator.reset()
                        val overlap = (fullBytes.size * 0.35).toInt().coerceAtMost(24000)
                        if (overlap > 0) {
                            audioAccumulator.write(fullBytes, fullBytes.size - overlap, overlap)
                        }
                        lastChunkSentTime = System.currentTimeMillis()

                        val seq = nextChunkSequence++
                        scope.launch {
                            groqProcessingMutex.withLock {
                                if (!isRunning || !scope.isActive || sessionId != currentSessionId || isDeepgramBackupActive) return@withLock
                                processGroqAudioChunk(fullBytes, sessionId, seq)
                            }
                        }
                    }
                }
    ...
    private suspend fun processGroqAudioChunk(audioBytes: ByteArray, sessionId: Long, chunkSeq: Long) {
        if (!isRunning || !scope.isActive || sessionId != currentSessionId || isDeepgramBackupActive) return
        val now = System.currentTimeMillis()

        val modelsToTry = if (groqTurboExhausted && !groqV3Exhausted) {
        ...
        var chunkCompletedSuccessfully = false
        var turboFailed = false
        var v3Failed = false
        ...
        for (model in modelsToTry) {
            ...
            if (isExhausted || inCooldown) {
                if (isTurbo) turboFailed = true else v3Failed = true
                continue
            }
            ...
            if (res.code == 200) {
                // Succeeded! (Even if text is empty because of broadcast silence/music)
                chunkCompletedSuccessfully = true
                groqConsecutiveFailures = 0
                ...
                // Client error (e.g. 400 Bad Request, unsupported format, 401 invalid key).
                // Do NOT mark as rate-limited/quota-exhausted and NEVER trigger paid Deepgram.
                android.util.Log.w("RadioStreamSttManager", "Groq $model rejected slice with HTTP ${res.code}. Skipping slice without triggering Deepgram.")
        ...
        if (chunkCompletedSuccessfully) {
            if (!transcriptionResult.isNullOrBlank()) {
                lastTranscriptTime = System.currentTimeMillis()
                val cleaned = sanitizeText(transcriptionResult)
                if (cleaned.length >= 2 && !isHallucinationLoop(cleaned)) {
                    groqPromptContext = cleaned.takeLast(180)
                    ingestSpeechTranscript(cleaned, isSpeechFinal = false)
                }
            }
        } else {
            // Only trigger Deepgram if BOTH Groq models failed on this slice with 429 / exhaustion / 5xx
            val bothGroqUnavailable = turboFailed && v3Failed
            if (bothGroqUnavailable) {
                val reason = if (groqTurboExhausted && groqV3Exhausted) {
                    "both_groq_quota_exhausted"
                } else {
                    "both_groq_rate_limited"
                }
                android.util.Log.w("RadioStreamSttManager", "[STT Fallback] DEEPGRAM_FALLBACK reason=$reason for chunk #$chunkSeq")
                activateDeepgramBackup(sessionId, reason)
            }
        }
    }
    ...
    private fun activateDeepgramBackup(sessionId: Long, reason: String = "both_groq_rate_limited") {
        ...
        android.util.Log.i("RadioStreamSttManager", "[STT Fallback] DEEPGRAM_FALLBACK reason=$reason for session #$sessionId")
        connectDeepgramWebSocket(sessionId)
    }
    ...
    private fun ingestSpeechTranscript(rawTranscript: String, isSpeechFinal: Boolean = false) {
    ...
    private fun handleDeepgramMessage(text: String) {
        try {
            ...
            ingestSpeechTranscript(rawTranscript, isSpeechFinal)
        } catch (e: Exception) {
    ...
    private fun formatSentenceCase(text: String): String {
        ...
        s = s.replaceFirstChar { if (it.isLowerCase()) it.titlecase(Locale.ROOT) else it.toString() }

        // 2. Capitalize after sentence terminators (. ? ! ;) followed by space
        s = s.replace(Regex("([.?!;]\\s+)([a-z])")) { mr ->
            mr.groupValues[1] + mr.groupValues[2].uppercase(Locale.ROOT)
        }

        // 3. Capitalize standalone 'i' and standard contractions
        s = s.replace(Regex("\\b(i)('m|'ve|'ll|'d)?\\b", RegexOption.IGNORE_CASE)) { mr ->
            "I" + (mr.groupValues[2].ifEmpty { "" })
        }

        // 4. Common dialogue starters after punctuation
        s = s.replace(Regex("([,;:]\\s+)(hi|hello|hey|yes|no|well|so|now|actually|certainly|indeed)\\b", RegexOption.IGNORE_CASE)) { mr ->
            mr.groupValues[1] + mr.groupValues[2].replaceFirstChar { if (it.isLowerCase()) it.titlecase(Locale.ROOT) else it.toString() }
        }
    ...
    private fun sanitizeText(input: String): String {
        ...
        return formatSentenceCase(cleaned)
    }
    ...
    private fun splitIntoLearningSentences(text: String): List<String> {
        ...
        for (s in rawSentences) {
            ...
            if (words.size <= 14) {
                learningSentences.add(formatSentenceCase(s.trim()))
            } else {
                // If a sentence is long (> 14 words), split cleanly at clause boundaries (comma, dash, colon)
                val clauseRegex = Regex("(?<=[,—:])\\s+")
                ...
                for (clause in clauses) {
                    if (acc.isEmpty()) {
                        ...
                        if (accWords.size >= 6) {
                            learningSentences.add(formatSentenceCase(acc.trim()))
                            acc = clause
                        } else {
                            acc = "$acc $clause"
                        }
