...
import { Clock, Volume2, VolumeX, Bookmark, BookmarkCheck, Tag, Gauge, Play, Pause, Repeat } from 'lucide-react';
...
import { normalizeAllCapitalizedSpeech, formatSentenceCase } from '../utils/textSanitizer';
...
export const BilingualSubtitleCard: React.FC<Props> = ({
  ...
  // Normalize any residual all-caps text into natural sentence case for comfortable reading
  const displayEnglish = useMemo(() => {
    return formatSentenceCase(normalizeAllCapitalizedSpeech(subtitle.english || ''));
  }, [subtitle.english]);
  ...
  const wordTokens = useMemo(() => {
    if (!displayEnglish) return [];
    const tokens = displayEnglish.split(/(\s+|[^\w'])/);
    return tokens;
  }, [displayEnglish]);
  ...
  const renderEnglishText = () => {
    ...
    const parts = displayEnglish.split(regex);
    ...
    >
      ...
      <div className="flex items-center justify-between mb-1.5 text-xs opacity-90">
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          {aheadOffset !== undefined && aheadOffset > 0 && !isInterim && !isLatest && (
            <span className="inline-flex items-center gap-1 font-semibold px-2 py-0.5 rounded-full text-[10px] sm:text-[11px] bg-cyan-500/15 text-cyan-600 dark:text-cyan-400 border border-cyan-500/30">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2">
          {/* Speed Toggle Pill (1.0x / 0.75x / 0.5x) - Omitted in YouTube Mode */}
          {!hideSpeedBookmark && (
            <button
              type="button"
          ...
          {/* Bookmark Button - Omitted in YouTube Mode */}
          {!hideSpeedBookmark && onBookmarkToggle && (
            <button
          ...
          {/* YouTube Mode: Enlarged Horizontal Video Play Button + Repeat Segment Toggle */}
          {hideSpeedBookmark ? (
            <div className="flex items-center gap-1.5 sm:gap-2">
              {/* Segment Repeat Toggle Button */}
              <button
                type="button"
                onClick={(e) => {
                  e.stopPropagation();
                ...
                title={isRepeatActive ? '點擊關閉此段落單曲循環' : '點擊開啟此段落單曲循環重複播放'}
                className={`h-7.5 sm:h-8.5 px-2 sm:px-2.5 rounded-xl transition-all cursor-pointer flex items-center justify-center gap-1 font-bold text-xs sm:text-xs shadow-sm active:scale-95 select-none whitespace-nowrap border ${
                  isRepeatActive
                    ? 'bg-amber-500/20 text-amber-600 dark:text-amber-300 border-amber-500/50 ring-2 ring-amber-400/60 shadow-amber-500/20 font-black'
                    : theme === 'paper'
                    ? 'bg-[#F4EAD5] text-[#7A6853] hover:text-[#3B2E1E] border-[#E0CFAB] hover:bg-[#E8D8B8]'
                    : theme === 'light'
                    ? 'bg-slate-100 text-slate-500 hover:text-slate-800 border-slate-200 hover:bg-slate-200'
                    : 'bg-slate-800/80 text-slate-400 hover:text-slate-200 border-slate-700/80 hover:bg-slate-700'
                }`}
              >
                <Repeat className={`w-3.5 h-3.5 shrink-0 ${isRepeatActive ? 'text-amber-500 animate-spin-slow' : 'opacity-70'}`} />
                <span className="text-[11px] sm:text-xs">{isRepeatActive ? '循環中' : '單句循環'}</span>
              </button>
              ...
              <button
                ...
                title={isPlayingSegment ? '暫停播放此段落' : '從此段落開始播放 YouTube 影片'}
                className={`h-7.5 sm:h-8.5 px-3.5 sm:px-4 rounded-xl transition-all cursor-pointer flex items-center justify-center gap-1.5 font-bold text-xs sm:text-sm shadow-md active:scale-95 select-none whitespace-nowrap ${
                  isPlayingSegment
                    ? 'bg-blue-600 hover:bg-blue-500 text-white ring-2 ring-blue-400 shadow-blue-500/40 animate-pulse'
                    : theme === 'paper'
                    ? 'bg-amber-800 hover:bg-amber-700 text-white border border-amber-900 shadow-amber-950/20'
                    : 'bg-blue-600 hover:bg-blue-500 text-white border border-blue-400/60 shadow-blue-600/30'
                }`}
              >
                {isPlayingSegment ? (
                  <>
            ...
            /* Radio Mode: Standard Audio Speak / Playback Button */
            <button
              onClick={handleSpeak}
