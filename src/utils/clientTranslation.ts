import { matchExactPhrase, postprocessChineseTranslation, preprocessEnglishForTranslation } from './broadcastGlossary';
...
export async function translateEnglishToChinese(text: string): Promise<string> {
  ...
  // 1. Google GTX
  try {
    const res = await withTimeout(translateWithGoogleGTX(preprocessedEn), 2200);
    if (res && !/^[a-zA-Z0-9\s.,!?'"-]+$/.test(res)) {
      const refined = postprocessChineseTranslation(res, cleanText);
  ...
  // 2. Google Clients5
  try {
    const res = await withTimeout(translateWithGoogleClients5(preprocessedEn), 1800);
    if (res && !/^[a-zA-Z0-9\s.,!?'"-]+$/.test(res)) {
      const refined = postprocessChineseTranslation(res, cleanText);
      translationCache.set(cleanText, refined);
      return refined;
  ...
  // 3. MyMemory
  try {
    const res = await withTimeout(translateWithMyMemory(preprocessedEn), 2200);
    if (res && !/^[a-zA-Z0-9\s.,!?'"-]+$/.test(res)) {
      const refined = postprocessChineseTranslation(res, cleanText);
      translationCache.set(cleanText, refined);
      return refined;
  ...
  const refinedFallback = postprocessChineseTranslation(fallback, cleanText);
  translationCache.set(cleanText, refinedFallback);
