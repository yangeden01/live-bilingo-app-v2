...
export type YouTubeNewsCategory = 'Breaking' | 'World' | 'US' | 'Business' | 'Technology';
...
export interface YouTubeNewsVideo {
  ...
  hasCaptions: boolean;
  captionBadge: string;
  category: YouTubeNewsCategory;
}

export interface YouTubeNewsResponse {
  success: boolean;
  videos: YouTubeNewsVideo[];
  cached?: boolean;
  timestamp?: number;
