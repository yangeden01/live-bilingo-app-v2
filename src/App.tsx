...
import { SubtitleItem, PlaybackStatus, RadioStation, ReadingMode, ChineseVariant, SubtitleFontSize, ContentSourceMode, APP_VERSION } from './types';
...
import { vibrateGitPushSuccess, vibrateZipExportSuccess, vibrateDetentTick } from './utils/haptics';
...
const DEFAULT_STATIONS: RadioStation[] = [
  {
    id: 'us-west-public-talk',
    name: '美西公共時事談話',
    freq: '加州地方時事',
    location: '加州舊金山灣區 (美西地區)',
    category: '美國西岸時事與公共訪談 (US West Coast Talk)',
    streamUrl: 'https://streams.kqed.org/kqedradio.mp3',
  },
  {
    id: 'us-midwest-news-talk',
    name: '中西部深度時事廣播',
    freq: '中西部深度專題',
    location: '伊利諾州芝加哥 (中西部)',
    category: '美國中西部時事與焦點訪談 (US Midwest Talk)',
    streamUrl: 'https://wbez.streamguys1.com/wbez128.mp3',
  },
  {
    id: 'us-northeast-public-news',
    name: '美東北公共英語新聞',
    freq: '新英格蘭生活時事',
  ...
  },
  {
    ...
    name: '全美綜合時事聯播',
    freq: '全美聯播新聞',
    location: '華盛頓哥倫比亞特區 (首都 / 全美)',
    category: '全美綜合時事與深度新聞 (US National Talk)',
  ...
  },
  {
    ...
    freq: '全球國際時事',
    location: '英國倫敦 (國際廣播)',
    category: '英國國際新聞與全球時事 (UK Global Talk)',
  ...
  },
  {
    ...
    freq: '商業財經焦點',
    location: '紐約曼哈頓 (財經中心)',
    category: '全美商業財經與市場焦點 (US Finance Talk)',
...
export default function App() {
  ...
  const [contentSource, setContentSource] = useState<ContentSourceMode>(() => {
    const saved = getPersistentItem('active_content_source') as ContentSourceMode;
  ...
  const [activeStation, setActiveStation] = useState<RadioStation>(() => {
    try {
      ...
      const savedStations = getPersistentItem('live_bilingo_stations_v4');
  ...
  useEffect(() => {
    try {
      if (stations && stations.length > 0) {
        setPersistentItem('live_bilingo_stations_v4', JSON.stringify(stations));
  ...
  // Global Chinese Variant State (Traditional / Simplified)
  const [chineseVariant, setChineseVariant] = useState<ChineseVariant>(() => {
    try {
  ...
  // Global Subtitle Font Size State (small / medium / large / xlarge)
  const [subtitleFontSize, setSubtitleFontSize] = useState<SubtitleFontSize>(() => {
    try {
  ...
  const {
    ...
    onTimerEnd: () => {
      // 1. Pause YouTube playback if running
      window.dispatchEvent(new CustomEvent('youtube-pause'));
      // 2. Pause radio playback if running
      ...
      setNotifToastMessage('🌙 睡眠定時結束，已自動停止播放');
      setTimeout(() => setNotifToastMessage(''), 4000);
    },
  });

  // CEFR difficulty highlight linked state between Radio and YouTube modes
  ...
  const handleHighlightDifficultyChange = useCallback((enabled: boolean) => {
    setHighlightDifficulty(enabled);
    try {
  ...
  const handleNextStation = useCallback(() => {
    ...
    const currentIdx = stations.findIndex((s) => s.id === activeStation.id);
  ...
  const handlePrevStation = useCallback(() => {
    ...
    setActiveStation(stations[prevIdx]);
  }, [stations, activeStation]);

  const handleCycleSleepTimer = useCallback(() => {
    // Minimum unit is 1 hour, maximum 5 hours: 0 -> 1h (60m) -> 2h (120m) -> 3h (180m) -> 4h (240m) -> 5h (300m) -> 0
    const steps = [0, 60, 120, 180, 240, 300];
    const currentIdx = steps.indexOf(sleepMinutes);
    const nextIdx = (currentIdx + 1) % steps.length;
    const nextMinutes = steps[nextIdx];
    selectSleepTimer(nextMinutes);
    const hours = nextMinutes / 60;
    setNotifToastMessage(
      hours > 0
        ? `🌙 睡眠定時已設定為 ${hours} 小時`
        : '🌙 睡眠定時器已關閉'
    );
    setTimeout(() => setNotifToastMessage(''), 3000);
  }, [sleepMinutes, selectSleepTimer]);

  // Synchronize Widget & AppWidget with current station, playback & timer
  useEffect(() => {
    if (typeof window !== 'undefined' && (window as any).AndroidBridge) {
      const isPlaying = playbackStatus === 'PLAYING' || playbackStatus === 'BUFFERING';
      const status = playbackStatus === 'BUFFERING'
        ? 'BUFFERING'
      ...
      if ((window as any).AndroidBridge.updateWidgetPlaybackStatus) {
        (window as any).AndroidBridge.updateWidgetPlaybackStatus(activeStation.name, isPlaying, sleepMinutes, status);
      } else if ((window as any).AndroidBridge.updateWidgetState) {
        (window as any).AndroidBridge.updateWidgetState(activeStation.name, isPlaying, sleepMinutes);
      }
      if ((window as any).AndroidBridge.updateSleepTimer) {
        (window as any).AndroidBridge.updateSleepTimer(sleepMinutes);
      }
    }
  }, [activeStation.name, playbackStatus, sleepMinutes]);

  // Listen to remote widget control events and Android sync events
  useEffect(() => {
    const onPrev = () => handlePrevStation();
    const onNext = () => handleNextStation();
    const onCycleTimer = () => handleCycleSleepTimer();

    const handleSyncWidgetMessage = (event: MessageEvent) => {
      if (event.data && event.data.type === 'SYNC_WIDGET_PLAYBACK') {
        const { isPlaying, stationName, sleepMinutes: remoteSleepMinutes } = event.data;
        if (stationName && stations.length > 0) {
          const matched = stations.find((s) => s.name === stationName);
          if (matched && matched.id !== activeStation.id) {
            setActiveStation(matched);
          }
        }
        if (typeof remoteSleepMinutes === 'number' && remoteSleepMinutes >= 0 && remoteSleepMinutes !== sleepMinutes) {
          selectSleepTimer(remoteSleepMinutes);
        }
        if (isPlaying) {
          window.dispatchEvent(new CustomEvent('app-foreground-wakeup-subtitles'));
        }
      }
    };

    window.addEventListener('radio-prev-station', onPrev);
    window.addEventListener('radio-next-station', onNext);
    window.addEventListener('radio-cycle-sleep-timer', onCycleTimer);
    window.addEventListener('message', handleSyncWidgetMessage);

    return () => {
      window.removeEventListener('radio-prev-station', onPrev);
      window.removeEventListener('radio-next-station', onNext);
      window.removeEventListener('radio-cycle-sleep-timer', onCycleTimer);
      window.removeEventListener('message', handleSyncWidgetMessage);
    };
  }, [handlePrevStation, handleNextStation, handleCycleSleepTimer, stations, activeStation.id, sleepMinutes, selectSleepTimer]);
    ...
    return [
      {
        ...
        isFinal: true,
        stationUrl: DEFAULT_STATIONS[0].streamUrl,
        stationName: DEFAULT_STATIONS[0].name,
      },
      {
        id: 'init-default-2',
        timestamp: new Date().toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', second: '2-digit' }),
  ...
  const handleNewSubtitle = useCallback((item: SubtitleItem) => {
    ...
    // Strict Playback Gate:
    // When radio playback is not active (paused or idle), do not append new live subtitles
    if (playbackStatusRef.current !== 'PLAYING' && playbackStatusRef.current !== 'BUFFERING') {
      return;
    }
  ...
  useEffect(() => {
    const handleCustomSubtitleEvent = (e: Event) => {
      ...
      if (customEvent.detail) {
        ...
        if (!item.stationUrl && activeStationRef.current?.streamUrl) {
          item.stationUrl = activeStationRef.current.streamUrl;
          item.stationName = activeStationRef.current.name;
        }
        handleNewSubtitle(item);
      }
    };
  ...
  const handleClearSubtitles = () => {
    ...
    safeApiFetch('/api/clear-subtitles-history', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ stationUrl: activeStation?.streamUrl }),
    });
  };
  ...
  const handleContentSourceChange = useCallback((newMode: ContentSourceMode) => {
    if (newMode === contentSource) return;

    if (typeof window !== 'undefined' && (window as any).AndroidBridge?.setContentMode) {
      try {
        (window as any).AndroidBridge.setContentMode(newMode);
      } catch (_) {}
    }

    if (newMode === 'youtube') {
      // Force pause radio playback when switching to YouTube mode to prevent audio overlap
      window.dispatchEvent(new CustomEvent('radio-force-pause'));
      if (typeof window !== 'undefined' && (window as any).AndroidBridge?.onStationPlaybackChanged) {
        try {
          (window as any).AndroidBridge.onStationPlaybackChanged('', '', false);
        } catch (_) {}
      }
      if (typeof window !== 'undefined' && (window as any).AndroidBridge?.stopBackupStt) {
        try {
          (window as any).AndroidBridge.stopBackupStt();
        } catch (_) {}
      }
  ...
  // YouTube Whole Video Loop Playback State (用戶需求：在YouTube模式，取消定時鍵改成整部video循環播放鍵)
  const [isYouTubeLoopEnabled, setIsYouTubeLoopEnabled] = useState<boolean>(() => {
    try {
      return localStorage.getItem('bilingo_youtube_loop_enabled') === 'true';
    } catch (_) {
      return false;
    }
  });

  const handleToggleYouTubeLoop = useCallback(() => {
    setIsYouTubeLoopEnabled((prev) => {
      const next = !prev;
      try {
        localStorage.setItem('bilingo_youtube_loop_enabled', String(next));
      } catch (_) {}
      window.dispatchEvent(new CustomEvent('youtube-loop-toggle', { detail: { enabled: next } }));
      vibrateDetentTick();
      return next;
    });
  }, []);

  // Smart Reading Mode Magnetic Detent Hook (strictly active for radio mode only)
  const { snapToReadingMode } = useReadingModeSnapDetent({
    enabled: activeTab === 'app' && contentSource === 'radio',
    targetId: 'subtitle-tabs-bar',
  ...
  const scrollToSubtitleReadingPosition = useCallback(() => {
    if (contentSource === 'youtube') return;
    setTimeout(() => {
      snapToReadingMode(true);
    }, 60);
  }, [snapToReadingMode, contentSource]);

  // Floating Watermark Button visibility:
  // 1. showBackToReadingFab (ArrowUp): shows when scrolled DOWN past reading mode (tabs scrolled up off screen)
  // 2. showScrollDownToReadingFab (ArrowDown): shows when screen is scrolled UP too high ("太置頂") to bring back down
  const [showBackToReadingFab, setShowBackToReadingFab] = useState(false);
  const [showScrollDownToReadingFab, setShowScrollDownToReadingFab] = useState(false);

  useEffect(() => {
    if (activeTab !== 'app') {
      setShowBackToReadingFab(false);
      setShowScrollDownToReadingFab(false);
      return;
    }
    ...
    const checkScrollState = () => {
      ...
      const rect = targetEl.getBoundingClientRect();
      // When target is scrolled above header by more than 20px: user is scrolled DOWN past reading mode
      const isPastReading = rect.top < headerBottom - 20;
      // When target is pushed below header by more than 50px: user is scrolled UP too high ("太置頂")
      const isTooHighAbove = rect.top > headerBottom + 50;

      setShowBackToReadingFab(isPastReading);
      setShowScrollDownToReadingFab(isTooHighAbove);
    };
  ...
  useEffect(() => {
    const handleScrollEvent = (e: Event) => {
      ...
      const isForced = customEvent?.detail?.force === true;
      // If user has deliberately scrolled down leaving latest paragraph (showBackToReadingFab is active),
      // suppress background auto-scroll events so user can read without jumping!
      if (isForced || !showBackToReadingFab) {
        scrollToSubtitleReadingPosition();
      }
    };
  ...
  const handleTopHeaderPlayToggle = () => {
    if (contentSource === 'youtube') {
      ...
      window.dispatchEvent(new CustomEvent('youtube-toggle-play'));
      // When entering playback, snap to YouTube reading position (video directly below header)
  ...
  return (
    <div className={`min-h-screen font-sans selection:bg-blue-600 selection:text-white pb-12 transition-colors duration-200 ${appBgClass}`}>
        ...
        <div className="max-w-6xl mx-auto px-3 sm:px-6 py-2 flex flex-nowrap items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
            {contentSource === 'youtube' ? (
              /* YouTube Mode: Whole Video Loop Button (用戶需求：在YouTube模式，取消定時鍵改成整個video循環播放鍵) */
              <button
                type="button"
                onClick={handleToggleYouTubeLoop}
                title={
                  isYouTubeLoopEnabled
                    ? '整部影片循環播放中（點擊取消循環）'
                    : '點擊開啟整部影片循環播放（影片結束自動重播）'
                }
                aria-label="YouTube整部影片循環播放切換"
                className={`px-2 sm:px-3 py-1.5 rounded-xl font-bold text-xs sm:text-sm flex items-center gap-1.5 transition-all active:scale-95 cursor-pointer border select-none ${
                  isYouTubeLoopEnabled
                    ? 'bg-emerald-600 text-white border-emerald-400 shadow-md ring-2 ring-emerald-400/40 font-black'
                    : effectiveTheme === 'paper'
                    ? 'bg-[#EFE6D2] text-[#3B2E1E] border-[#D8C49E] hover:bg-[#E5D7BF]'
                    : effectiveTheme === 'light'
                    ? 'bg-slate-100 text-slate-700 border-slate-300 hover:bg-slate-200'
                    : 'bg-slate-800 text-slate-200 border-slate-700 hover:bg-slate-700'
                }`}
              >
                <Repeat className={`w-3.5 h-3.5 shrink-0 ${isYouTubeLoopEnabled ? 'stroke-[2.5] text-white animate-pulse' : ''}`} />
                <span>{isYouTubeLoopEnabled ? '循環中' : '循環'}</span>
              </button>
            ) : (
              /* Radio Mode: Sleep Timer Trigger Button */
              <button
                type="button"
                onClick={() => setIsSleepTimerModalOpen(true)}
                title={
                  remainingSeconds !== null && remainingSeconds > 0
                    ? `睡眠定時倒數中（剩餘 ${Math.floor(remainingSeconds / 3600) > 0 ? `${Math.floor(remainingSeconds / 3600)}小時` : ''}${Math.ceil((remainingSeconds % 3600) / 60)}分鐘），點擊修改或關閉`
                    : sleepMinutes > 0
                    ? `已設定睡眠定時 ${sleepMinutes} 分鐘，點擊修改或關閉`
                    : '點擊設定睡眠自動關閉定時器 (30秒平滑淡出)'
                }
                className={`px-2 sm:px-3 py-1.5 rounded-xl font-bold text-xs sm:text-sm flex items-center gap-1.5 transition-all active:scale-95 cursor-pointer border select-none ${
                  remainingSeconds !== null && remainingSeconds > 0
                    ? 'bg-amber-600/90 text-amber-50 border-amber-400/80 shadow-md ring-2 ring-amber-400/40 font-black'
                    : sleepMinutes > 0
                    ? 'bg-indigo-600 text-white border-indigo-400 shadow-md ring-2 ring-indigo-400/40 font-black'
                    : effectiveTheme === 'paper'
                    ? 'bg-[#EFE6D2] text-[#3B2E1E] border-[#D8C49E] hover:bg-[#E5D7BF]'
                    : effectiveTheme === 'light'
                    ? 'bg-slate-100 text-slate-700 border-slate-300 hover:bg-slate-200'
              ...
              >
                <Moon className={`w-3.5 h-3.5 shrink-0 ${remainingSeconds !== null && remainingSeconds > 0 ? 'fill-current text-amber-200 animate-pulse' : ''}`} />
                {remainingSeconds !== null && remainingSeconds > 0 ? (
            ...
            {/* Top Sticky Play/Pause Button (Far right for single-handed operation) */}
            <button
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 pt-4 pb-12 pb-[max(3rem,env(safe-area-inset-bottom))] space-y-6">
        ...
        {activeTab === 'app' && (
          <>
            <div className={contentSource === 'radio' ? 'contents' : 'hidden'}>
              {/* Global Radio Stream Controller */}
              <AudioPlayerController
                isActive={contentSource === 'radio'}
            ...
            <div className={contentSource === 'youtube' ? 'contents' : 'hidden'}>
              <YouTubeBilingualView
                ...
                onSwitchToRadioMode={() => handleContentSourceChange('radio')}
              />
            </div>
          </>
        )}
          ...
          <div className="bg-slate-900 rounded-2xl p-6 border border-slate-800 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 space-y-2">
                ...
                <h4 className="font-bold text-sm text-white">Groq Whisper Large V3 Turbo</h4>
                <p className="text-xs text-slate-400 leading-relaxed">
                  Groq LPU 雲端晶片極速聽寫（$0.04/hr）：<br />
                  <code className="text-slate-300 font-mono text-[10px] block mt-1 break-all bg-slate-900 p-1.5 rounded">
                    model=whisper-large-v3-turbo (Groq Cloud API)
                  </code>
                  備用降級：<code className="text-emerald-400 font-mono">Deepgram Nova-2 WebSocket</code>
                </p>
              </div>
      ...
      {/* Floating Watermark Back-to-Reading-Mode Buttons (置頂分頁: 下拉離開時出現上移icon; 閱讀模式: 上拉太置頂時出現下移icon) */}
      <AnimatePresence>
        {activeTab === 'app' && (showBackToReadingFab || showScrollDownToReadingFab) && (
          <motion.button
            key={showBackToReadingFab ? 'back-to-reading-up' : 'back-to-reading-down'}
            initial={{ opacity: 0, scale: 0.6, y: showBackToReadingFab ? 20 : -20 }}
            animate={{ opacity: 0.92, scale: 1, y: 0 }}
            exit={{ opacity: 0, scale: 0.6, y: showBackToReadingFab ? 20 : -20 }}
            whileHover={{ opacity: 1, scale: 1.06 }}
            whileTap={{ scale: 0.92 }}
            transition={{ type: 'spring', stiffness: 450, damping: 26 }}
            onClick={handleReturnToReadingMode}
            title={
              showBackToReadingFab
                ? (contentSource === 'youtube' ? '回到 YouTube 雙語閱讀與影音播放位置' : '回到閱讀模式（置頂「即時 / 紀錄 / 收藏 / 生詞」分頁）')
                : (contentSource === 'youtube' ? '拉回 YouTube 雙語閱讀視窗' : '拉回閱讀模式（置頂播放與字幕區）')
            }
            aria-label="回到雙語閱讀模式"
            id={showBackToReadingFab ? 'back-to-reading-mode-fab' : 'scroll-down-to-reading-fab'}
          ...
          >
            {showBackToReadingFab ? (
              ...
              <>
                <ArrowDown className="w-5 h-5 text-emerald-500 dark:text-emerald-400 group-hover:translate-y-0.5 transition-transform duration-200" />
                <span className="text-[9px] sm:text-[10px] font-black tracking-tighter leading-none opacity-90 group-hover:opacity-100 whitespace-nowrap">
                  閱讀模式
                </span>
              </>
      ...
      <SleepTimerModal
        ...
        onSelectMinutes={selectSleepTimer}
