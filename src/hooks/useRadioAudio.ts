...
export function useRadioAudio({
  ...
  const needsSubtitleWakeupOnForegroundRef = useRef<boolean>(false);
  ...
  useEffect(() => {
    ...
    if (isActiveRef.current && (playbackStatus === 'PLAYING' || playbackStatus === 'BUFFERING')) {
      userPlaybackIntentRef.current = true;
    } else if (!isActiveRef.current || playbackStatus === 'PAUSED' || playbackStatus === 'IDLE') {
      userPlaybackIntentRef.current = false;
    }
  }, [playbackStatus]);
  ...
  // Ensure any legacy 30s buffer flag is cleared to prevent buffering stalls
  useEffect(() => {
    try {
      if (typeof localStorage !== 'undefined') {
        localStorage.removeItem('radio_30s_buffer_mode');
      }
    } catch (e) {}
  }, []);
  ...
  const handleAutoReconnect = useCallback(() => {
    if (!isActiveRef.current || !audioRef.current || isReconnectingRef.current) return;
  ...
  const triggerFreshStreamReconnect = useCallback(() => {
    if (!isActiveRef.current || !audioRef.current || !userPlaybackIntentRef.current) return;
    const audio = audioRef.current;
    isReconnectingRef.current = false;
    retryCountRef.current = 0;
  ...
  useEffect(() => {
    ...
    const handleVisibility = () => {
      // STRICT ISOLATION: If radio view is not active, NEVER auto-resume or touch radio stream
      if (!isActiveRef.current) {
        return;
      }
      ...
      if (document.visibilityState === 'visible') {
        ...
        try {
          if ((window as any).AndroidBridge?.getWidgetPlaybackState) {
            const raw = (window as any).AndroidBridge.getWidgetPlaybackState();
            const parsed = JSON.parse(raw || '{}');
            if (parsed.isPlaying) isNativePlaying = true;
          }
        } catch (e) {}

        const shouldResume = userPlaybackIntentRef.current || isNativePlaying || needsSubtitleWakeupOnForegroundRef.current;
        if (shouldResume) {
          userPlaybackIntentRef.current = true;
          needsSubtitleWakeupOnForegroundRef.current = false;
          if (playbackStatusRef.current !== 'PLAYING') {
            playbackStatusRef.current = 'PLAYING';
            setPlaybackStatus('PLAYING');
          }
          ...
          // Seamlessly trigger subtitle recovery
    ...
    window.addEventListener('radio-force-pause', handleForcePause);
    ...
    return () => {
      ...
      window.removeEventListener('radio-force-pause', handleForcePause);
  ...
  useEffect(() => {
    ...
    // Set playback intent so watchdog monitors and keeps stream active
    userPlaybackIntentRef.current = true;
    playbackStatusRef.current = 'BUFFERING';
    setPlaybackStatus('BUFFERING');

    // Clear stale subtitle queues from previous station
    onClearSubtitleQueueRef.current?.();
    fetch(getApiUrl('/api/clear-buffer'), { method: 'POST' }).catch(() => {});

    // Notify backend to immediately initialize new station STT session
    safeApiFetch('/api/notify-station-playing', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    // Notify Android native layer to switch STT stream immediately
    try {
      if ((window as any).AndroidBridge?.onStationPlaybackChanged) {
        (window as any).AndroidBridge.onStationPlaybackChanged(
  ...
  const pausePlaybackAction = useCallback(() => {
    userPlaybackIntentRef.current = false;
    playbackStatusRef.current = 'PAUSED';
    setPlaybackStatus('PAUSED');
    if (audioRef.current) {
  ...
  const playPlaybackAction = useCallback(() => {
    userPlaybackIntentRef.current = true;
    needsSubtitleWakeupOnForegroundRef.current = false;
    if (playbackStatusRef.current !== 'PLAYING') {
      playbackStatusRef.current = 'PLAYING';
    ...
    window.dispatchEvent(new CustomEvent('app-foreground-wakeup-subtitles'));
  }, [safePlay, setPlaybackStatus]);

  // Expose global direct control API on window for native Android (Widget, Notifications, LockScreen)
  useEffect(() => {
    let lastControlTime = 0;

    (window as any).radioControl = {
      play: () => {
        playPlaybackAction();
      },
      pause: () => {
      ...
      toggle: () => {
        const now = Date.now();
        if (now - lastControlTime < 350) return;
        lastControlTime = now;
      ...
      prevStation: () => {
        window.dispatchEvent(new CustomEvent('radio-prev-station'));
      },
      nextStation: () => {
        window.dispatchEvent(new CustomEvent('radio-next-station'));
      },
      cycleSleepTimer: () => {
        window.dispatchEvent(new CustomEvent('radio-cycle-sleep-timer'));
      },
      isPlaying: () =>
        playbackStatusRef.current === 'PLAYING' ||
        playbackStatusRef.current === 'BUFFERING' ||
    ...
    return () => {
      delete (window as any).radioControl;
    };
  }, [pausePlaybackAction, playPlaybackAction, togglePlayPause]);
    ...
    const handleAndroidMediaMessage = (event: MessageEvent) => {
      ...
      if (event.data && event.data.type === 'ANDROID_MEDIA_CONTROL') {
        ...
        if (action === 'com.bilingo.radio.ACTION_TOGGLE_PLAY') {
          const isCurrentlyPlaying = playbackStatusRef.current === 'PLAYING' || playbackStatusRef.current === 'BUFFERING' || (audioRef.current && !audioRef.current.paused);
          if (isCurrentlyPlaying) {
            pausePlaybackAction();
          } else {
            const isAppHidden = typeof document !== 'undefined' && document.visibilityState === 'hidden';
            togglePlayPause();
            // When started from background / widget, keep STT asleep to save Groq quota ($0/hr)
            if (isAppHidden) {
              needsSubtitleWakeupOnForegroundRef.current = true;
              safeApiFetch('/api/subtitle-stream-sleep', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ reason: 'widget_background_play' }),
              }).catch(() => {});
            }
          }
        ...
        } else if (action === 'com.bilingo.radio.ACTION_PREV_STATION') {
          window.dispatchEvent(new CustomEvent('radio-prev-station'));
        } else if (action === 'com.bilingo.radio.ACTION_NEXT_STATION') {
          window.dispatchEvent(new CustomEvent('radio-next-station'));
        } else if (action === 'com.bilingo.radio.ACTION_SLEEP_TIMER') {
          window.dispatchEvent(new CustomEvent('radio-cycle-sleep-timer'));
        }
      }
    };
  ...
  }, [pausePlaybackAction, playPlaybackAction, setPlaybackStatus, togglePlayPause]);
