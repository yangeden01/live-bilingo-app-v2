...
// Render Chinese example sentence with target Chinese translation highlighted in matching prominent color
function renderHighlightedZhSentence(
  sentence: string,
  targetZhRaw?: string,
  englishWord?: string
) {
  ...
  // Extract clean Chinese search tokens from targetZhRaw
  const cleanZh = (targetZhRaw || '')
    .replace(/^[a-zA-Z\.\s]+[:：]?\s*/, '')
    .replace(/^\[[^\]]+\]\s*/, '')
    .replace(/^\([^\)]+\)\s*/, '');

  const tokens = cleanZh
    .split(/[/,，、;\s]+/)
    .map((t) => t.trim().replace(/^[^\u4e00-\u9fa5]+|[^\u4e00-\u9fa5]+$/g, ''))
    .filter((t) => t.length > 0);

  const primaryZh = tokens[0] || '';

  // If the Chinese sentence inadvertently contains the English word (e.g. "關於access的最新進展"),
  // replace it with the Chinese word first!
  if (englishWord && primaryZh) {
    const cleanEn = cleanWordToken(englishWord);
    if (cleanEn) {
      const enRegex = new RegExp(`\\b${cleanEn}\\b`, 'gi');
      displaySentence = displaySentence.replace(enRegex, primaryZh);
    }
  }

  // Find all matched Chinese keywords to highlight
  // Sort by length descending to match longer phrases first
  const matchCandidates = Array.from(
    new Set(tokens.filter((t) => t.length >= 2 || (t.length === 1 && displaySentence.includes(t))))
  );
  matchCandidates.sort((a, b) => b.length - a.length);

  if (matchCandidates.length === 0 && primaryZh.length >= 2) {
    matchCandidates.push(primaryZh);
  }

  const validCandidates = matchCandidates.filter((c) => displaySentence.includes(c));
  if (validCandidates.length === 0) {
    return <span>{displaySentence}</span>;
  }

  const escapedCandidates = validCandidates.map((c) => c.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'));
  const regex = new RegExp(`(${escapedCandidates.join('|')})`, 'g');

  const segments: Array<{ text: string; isMatch: boolean }> = [];
  let lastIndex = 0;
  let match: RegExpExecArray | null;

  while ((match = regex.exec(displaySentence)) !== null) {
    const matchStart = match.index;
    const matchEnd = regex.lastIndex;
    if (matchStart > lastIndex) {
      segments.push({ text: displaySentence.slice(lastIndex, matchStart), isMatch: false });
    }
  ...
  if (lastIndex < displaySentence.length) {
    segments.push({ text: displaySentence.slice(lastIndex), isMatch: false });
  }
...
export const DictionaryModal: React.FC<Props> = ({ isOpen, onClose, initialWord = '' }) => {
              ...
              for (const item of dmList) {
                if (item.defs && item.defs.length > 0) {
                  ...
                  const meanings: DictionaryMeaning[] = item.defs.slice(0, 4).map((dStr: string, idx: number) => {
                    ...
                    const contextual = generateContextualExample(cand, pos, idx, defText, usedSentences, lookupQuickWord(cand)?.zh || lookupQuickWord(clean)?.zh);
                    return {
                      partOfSpeech: pos,
                      definition: defText,
              ...
              if (Array.isArray(jsonArr) && jsonArr.length > 0) {
                ...
                for (let idx = 0; idx < rawMeanings.length; idx++) {
                  ...
                  if (ex) {
                    ...
                    const contextual = generateContextualExample(cand, pos, idx, def, usedSentences, lookupQuickWord(cand)?.zh || lookupQuickWord(clean)?.zh);
  ...
  const toggleSaveWord = (word: string) => {
    ...
    const isSaved = currentSaved.some((w) => w.toLowerCase() === clean.toLowerCase());
    const updated = isSaved
      ? currentSaved.filter((w) => w.toLowerCase() !== clean.toLowerCase())
      : [clean, ...currentSaved.filter((w) => w.toLowerCase() !== clean.toLowerCase())];
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-start justify-between bg-blue-50/70 dark:bg-blue-950/40 p-4 rounded-2xl border border-blue-100 dark:border-blue-900/50 shadow-sm">
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    ...
                    >
                      {savedWords.some((w) => w.toLowerCase() === result.word.toLowerCase()) ? (
                        ...
                        {isDetailsLoading ? (
                          ...
                          (() => {
                            ...
                            return detailedMeanings.map((m, idx) => {
                              ...
                              let targetZhWord = m.chineseTranslation || result?.chineseTranslation || lookupQuickWord(result?.word || searchWord)?.zh || '';
                              ...
                              if (!effectiveExample || renderedSentencesSet.has(effectiveExample)) {
                                ...
                                if (generated.targetZh) {
                                  targetZhWord = generated.targetZh;
                                }
                              } else {
