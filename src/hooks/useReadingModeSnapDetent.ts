...
export function useReadingModeSnapDetent({
  ...
  useEffect(() => {
    ...
    const handleTouchEnd = () => {
      ...
      // IMPORTANT: Only snap when stopping near or above targetY. If the user has scrolled down into the subtitle
      // list to read older paragraphs (currentScrollY > targetY + 15), NEVER snap back, allowing undisturbed reading!
      if (currentScrollY <= targetY + 15 && dist <= snapThresholdPx && velocity < velocityThreshold) {
    ...
    const handleScroll = () => {
      ...
      scrollTimeoutRef.current = setTimeout(() => {
        ...
        // Only snap if stopped very close to reading mode AND not scrolled down reading older paragraphs
        if (currentScrollY <= targetY + 15 && dist > 4 && dist <= 40) {
          snapToReadingMode(true);
        }
      }, 120);
    };
