...
import { isStationUrlMatch } from './stationHelper';
...
class ClientSubtitleEngine {
  ...
  private async fetchLiveSubtitles() {
    ...
    try {
      const encodedStation = encodeURIComponent(this.activeStation.streamUrl);
      const url = getApiUrl(`/api/live-subtitles?since=${this.lastPollTimestamp}&stationUrl=${encodedStation}`);
